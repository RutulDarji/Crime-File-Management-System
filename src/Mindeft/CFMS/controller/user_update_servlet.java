		...
		try {
			...
			for (int i = 0; i < items.size(); i++)
			{
				...
				if (item.isFormField()) 
				{
					...
					if (fieldName.equalsIgnoreCase("isActive")) 
					{
						isActive = fieldValue;		
					...
					if (fieldName.equalsIgnoreCase("user_type_id")) 
					{
						user_type_id = fieldValue;				
					...
					if (fieldName.equalsIgnoreCase("otp")) 
					{
						otp = fieldValue;				
						System.out.println("otp : " + otp);
						System.out.println("Original otp : " + send_otp.getotp());
						
						if(StringValidationUtility.isEmpty(otp))
						{
							req.setAttribute("otp", "<font color=red>*Enter Valid otp</font>");
							isError=true;
						}
						else
						{
							if(send_otp.getotp().equals(otp))
							{
					...
					if (fieldName.equalsIgnoreCase("password")) 
					{
						...
						{
							req.setAttribute("password", "<font color=red>*Enter Valid password</font>");
							isError=true;
						}
					}
			...
			else
			{
				...
				userBeanObj.setUser_id(user_id);
				
				
				if(userDao.update(userBeanObj));
				{
